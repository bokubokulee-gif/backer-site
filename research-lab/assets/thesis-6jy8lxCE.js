import"./styles-B29Xo75-.js";const W=`# Backer Research Thesis #003
...
`,m=document.querySelector("#article-body"),L=document.querySelector("#article-title"),C=document.querySelector("#article-deck"),A=document.querySelector("#article-toc"),O=document.querySelector("#reading-time"),_=document.querySelector("#reading-progress-bar"),b=Array.from(document.querySelectorAll("[data-document]")),$={essay:{title:"Where Attention Goes Next",deck:"Attention becomes valuable before it becomes obvious. Backer studies where it moves, what changes its path, and how reality corrects the forecast.",source:H,label:"Essay"},protocol:{title:"Predicting human attention flow",deck:"A research program for forecasting attention trajectories, separating prediction from intervention, and testing when costly commitment adds useful signal.",source:W,label:"Research protocol"}};let p="essay",j=null;function g(o){return String(o).replaceAll("&","&amp;").replaceAll("<","&lt;").replaceAll(">","&gt;").replaceAll('"',"&quot;").replaceAll("'","&#039;")}function T(o){const t=[];let e=g(o).replace(/`([^`]+)`/g,(a,r)=>{const n=`@@CODE${t.length}@@`;return t.push(`<code>${r}</code>`),n});return e=e.replace(/\[([^\]]+)\]\(([^)]+)\)/g,(a,r,n)=>{const i=/^https?:\/\//i.test(n);return`<a href="${n}"${i?' target="_blank" rel="noopener noreferrer"':""}>${r}${i?'<span class="external-mark" aria-hidden="true">&#8599;</span>':""}</a>`}).replace(/\*\*([^*]+)\*\*/g,"<strong>$1</strong>").replace(/(^|[^*])\*([^*]+)\*/g,"$1<em>$2</em>"),t.forEach((a,r)=>{e=e.replace(`@@CODE${r}@@`,a)}),e}function F(o,t){const e=o.toLowerCase().replace(/<[^>]+>/g,"").replace(/[^a-z0-9]+/g,"-").replace(/^-|-$/g,"")||"section",a=t.get(e)||0;return t.set(e,a+1),a?`${e}-${a+1}`:e}function z(o){const t=o.replace(/\r\n/g,`
`).split(`
`),e=[],a=[],r=new Map;let n=[],i=null,v=[],y=[],f=!1,w="",S=[];const l=()=>{n.length&&(e.push(`<p>${T(n.join(" "))}</p>`),n=[])},c=()=>{!v.length||!i||(e.push(`<${i}>${v.map(u=>`<li>${T(u)}</li>`).join("")}</${i}>`),v=[],i=null)},d=()=>{y.length&&(e.push(`<blockquote><p>${T(y.join(" "))}</p></blockquote>`),y=[])},E=()=>{const u=w?`<span>${g(w)}</span>`:"";e.push(`<div class="article-code">${u}<pre><code>${g(S.join(`
`))}</code></pre></div>`),S=[],w=""};return t.forEach(u=>{const s=u.trimEnd();if(s.startsWith("```")){f?(E(),f=!1):(l(),c(),d(),f=!0,w=s.slice(3).trim());return}if(f){S.push(u);return}const I=s.match(/^(#{1,3})\s+(.+)$/);if(I){l(),c(),d();const h=I[1].length,k=I[2].trim();if(h===1)return;const P=F(k,r);a.push({level:h,label:k,id:P}),e.push(`<h${h} id="${P}"><a href="#${P}" aria-label="Link to ${g(k)}">${T(k)}</a></h${h}>`);return}if(/^>\s?/.test(s)){l(),c(),y.push(s.replace(/^>\s?/,""));return}const B=s.match(/^[-*]\s+(.+)$/),R=s.match(/^\d+\.\s+(.+)$/);if(B||R){l(),d();const h=B?"ul":"ol";i&&i!==h&&c(),i=h,v.push((B||R)[1]);return}if(!s.trim()){l(),c(),d();return}if(/^---+$/.test(s.trim())){l(),c(),d(),e.push("<hr />");return}d(),c(),n.push(s.trim())}),f&&E(),l(),c(),d(),{html:e.join(`
`),headings:a}}function D(o){A&&(A.innerHTML=o.filter(t=>t.level===2).map((t,e)=>`<a href="#${t.id}" data-toc-id="${t.id}"><span>${String(e+1).padStart(2,"0")}</span>${g(t.label)}</a>`).join(""),A.querySelectorAll("a").forEach(t=>{t.addEventListener("click",e=>{const a=document.querySelector(t.getAttribute("href"));a&&(e.preventDefault(),a.scrollIntoView({behavior:window.matchMedia("(prefers-reduced-motion: reduce)").matches?"auto":"smooth",block:"start"}),history.replaceState(null,"",t.getAttribute("href")))})}))}function K(){j?.disconnect();const o=Array.from(m?.querySelectorAll("h2")||[]);!o.length||!("IntersectionObserver"in window)||(j=new IntersectionObserver(t=>{const e=t.filter(a=>a.isIntersecting).sort((a,r)=>r.intersectionRatio-a.intersectionRatio)[0];e&&A?.querySelectorAll("a").forEach(a=>{const r=a.dataset.tocId===e.target.id;a.classList.toggle("is-active",r),r?a.setAttribute("aria-current","location"):a.removeAttribute("aria-current")})},{rootMargin:"-18% 0px -70% 0px",threshold:[0,.15,.5]}),o.forEach(t=>j.observe(t)))}function G(o){const t=o.replace(/```[\s\S]*?```/g," ").replace(/[#>*_`\[\]()/-]/g," ").trim().split(/\s+/).filter(Boolean).length;return Math.max(1,Math.ceil(t/220))}function x(o,t={}){const e=$[o]||$.essay;p=o in $?o:"essay";const a=z(e.source);L&&(L.textContent=e.title),C&&(C.textContent=e.deck),m&&(m.innerHTML=a.html),O&&(O.textContent=`${G(e.source)} min`),document.title=`${e.title} | Backer Research Lab`,b.forEach(r=>{const n=r.dataset.document===p;r.setAttribute("aria-selected",n?"true":"false"),r.tabIndex=n?0:-1}),D(a.headings),K(),q(),t.updateHistory!==!1&&history.pushState({document:p},"",`#${p}`),t.focusArticle&&document.querySelector(".reader-boundary")?.scrollIntoView({behavior:window.matchMedia("(prefers-reduced-motion: reduce)").matches?"auto":"smooth",block:"start"})}b.forEach((o,t)=>{o.addEventListener("click",()=>{x(o.dataset.document,{focusArticle:!0})}),o.addEventListener("keydown",e=>{if(!["ArrowLeft","ArrowRight"].includes(e.key))return;e.preventDefault();const a=e.key==="ArrowRight"?1:-1,r=b[(t+a+b.length)%b.length];r.focus(),x(r.dataset.document)})});function q(){if(!m||!_)return;const o=m.getBoundingClientRect(),t=window.scrollY+o.top-window.innerHeight*.3,e=t+m.offsetHeight-window.innerHeight*.45,a=e<=t?0:(window.scrollY-t)/(e-t);_.style.transform=`scaleX(${Math.max(0,Math.min(1,a))})`}let M=!1;window.addEventListener("scroll",()=>{M||(M=!0,window.requestAnimationFrame(()=>{q(),M=!1}))},{passive:!0}),window.addEventListener("resize",q,{passive:!0}),window.addEventListener("popstate",()=>{const o=location.hash==="#protocol"?"protocol":location.hash==="#essay"?"essay":p;o!==p&&x(o,{updateHistory:!1})}),document.querySelector("#print-article")?.addEventListener("click",()=>window.print());const N=location.hash==="#protocol"?"protocol":"essay";x(N,{updateHistory:!1});
