...
The language control sits beside the brand in the upper-left header. Pages without a brand header use an upper-left control. A choice persists on that deployment and travels in the `lang` query parameter; internal navigation stays on the current host. Switching languages reloads the current route so text animations and page controllers initialize against the selected language.
