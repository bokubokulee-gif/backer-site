(function () {
  ...
  function renderResponse() {
    ...
    window.dispatchEvent(new CustomEvent('backer:hft-selection', { detail: { eventId: state.event, profileId: state.profile, historyIncluded: state.history } }));
  ...
  var STAGES = {
    setup: { kind: '01 / The connection', title: 'One model. A precise interface.', body: 'Install the TypeSafe skill and Python SDK. Keep the API key on the server, use the direct endpoint, and pin a model version so every experiment can be reproduced.', note: 'This reference walkthrough uses authored examples. The separate local adapter can request a Jev judgment.' },
    ...
    policy: { kind: '04 / The policy', title: 'The model judges. Code decides what happens.', body: 'The simulator checks freshness and feasibility before using an answer. Cash, holdings and scenario rules remain deterministic. A fund consuming the signal keeps its own exposure limits, risk vetoes and execution policy.', note: 'Confidence thresholds require evaluation on held-out decisions. A concentrated distribution is not proof that a prediction is correct.' },
    advance: { kind: '05 / The next situation', title: 'Update the state. Run the next decision.', body: 'Apply a simulated fill, account for costs, update holdings and cash, then advance the market clock. Send the new state in a separate Jev request. Repeat across individuals and paths to build the population response.', note: 'The participant ledger uses fixed ten-share fills without fees. The separate fund comparison illustrates costs and partial fills. Research runs must also evaluate impact and missed fills. Simulated actions never become observed history.' },
