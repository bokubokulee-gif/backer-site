(()=>{"use strict";const c=document.querySelector("[data-research-cards]");if(!c)return;const f=document.querySelector("[data-research-status]"),b={flow:{url:"research-lab/attention-flow.html",title:"Attention flow: interactive illustrative field",ready:"#attention-canvas"},simulation:{url:"research-lab/attention-simulation.html",title:"Attention simulation: interactive Backer World",ready:"iframe"},trading:{url:"research-lab/",title:"Trading behavior: interactive modeled attention field",ready:"#scene-mount"}},d=`
  ...
  `,x={flow:`${d}
    ...
    `},s=Array.from(c.querySelectorAll("[data-research-card]")),l=new Map;s.forEach(e=>{const t=e.dataset.researchCard,a=b[t];a&&l.set(e,{key:t,study:a,frame:null,timer:0,resizeFrame:0,nestedStarted:!1})});function h(e){const t=l.get(e);if(!(!t||!t.frame))try{const a=t.frame.contentWindow;a.dispatchEvent(new a.Event("resize"))}catch{}}function u(e){const t=l.get(e);if(!t||t.frame)return;const a=e.querySelector("[data-research-window]"),p=e.querySelector("[data-research-mount]"),i=e.querySelector("[data-research-loading]"),v=e.querySelector("[data-research-loading-copy]"),n=()=>{a.setAttribute("aria-busy","false"),v.textContent="Open the full preview to explore this research.",i.querySelector(".p2r-loading-line").hidden=!0},r=document.createElement("iframe");t.frame=r,r.className="p2r-frame",r.title=t.study.title,r.allow="fullscreen",r.setAttribute("referrerpolicy","strict-origin-when-cross-origin");const w=()=>{window.clearTimeout(t.timer),a.classList.add("is-ready"),a.setAttribute("aria-busy","false"),i.hidden=!0,f&&(f.textContent=`${t.study.title} loaded.`),window.requestAnimationFrame(()=>h(e))};r.addEventListener("error",n),r.addEventListener("load",()=>{let g=!1;try{const o=r.contentDocument;if(o&&o.head&&o.querySelector(t.study.ready)){const y=o.createElement("style");if(y.textContent=x[t.key],o.head.appendChild(y),t.key==="simulation"){if(t.nestedStarted)return;t.nestedStarted=!0;const m=o.querySelector("iframe");m.addEventListener("load",w,{once:!0}),m.addEventListener("error",n,{once:!0}),window.clearTimeout(t.timer),t.timer=window.setTimeout(n,18e3),m.src=m.getAttribute("src");return}g=!0}}catch{}if(!g){n();return}w()}),r.src=t.study.url,p.append(r),t.timer=window.setTimeout(n,18e3)}if("ResizeObserver"in window){const e=new ResizeObserver(t=>{t.forEach(a=>{const p=a.target.closest("[data-research-card]"),i=l.get(p);i&&(window.cancelAnimationFrame(i.resizeFrame),i.resizeFrame=window.requestAnimationFrame(()=>h(p)))})});s.forEach(t=>e.observe(t.querySelector("[data-research-window]")))}if("IntersectionObserver"in window){const e=new IntersectionObserver(t=>{t.forEach(a=>{a.isIntersecting&&(u(a.target),e.unobserve(a.target))})},{threshold:.01,rootMargin:"250px 0px"});s.forEach(t=>e.observe(t))}else s.forEach(u)})();
